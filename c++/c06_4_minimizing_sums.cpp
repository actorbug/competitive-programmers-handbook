...
namespace {
	...
	double min2(const vector<double>& ary) {
		return ranges::fold_left(ary, 0, plus()) / ary.size();
	}
...
TEST(C064MinimizingSums, min1sum1) {
	EXPECT_DOUBLE_EQ(min1({ 1 }), 1);
	EXPECT_DOUBLE_EQ(sum1({ 1 }, min1({ 1 })), 0);
	EXPECT_DOUBLE_EQ(min1({ 1, 2 }), 2);
	EXPECT_DOUBLE_EQ(sum1({ 1, 2 }, min1({ 1, 2 })), 1);
	EXPECT_DOUBLE_EQ(min1({ 1, 2, 9, 2, 6 }), 2);
	EXPECT_DOUBLE_EQ(sum1({ 1, 2, 9, 2, 6 }, min1({ 1, 2, 9, 2, 6 })), 12);
}

TEST(C064MinimizingSums, min2sum2) {
	EXPECT_DOUBLE_EQ(min2({ 1 }), 1);
	EXPECT_DOUBLE_EQ(sum2({ 1 }, min2({ 1 })), 0);
	EXPECT_DOUBLE_EQ(min2({ 1, 2, 9, 2, 6 }), 4);
